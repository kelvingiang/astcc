...
class Model_Check_In_Event_Function
{
    ...
    public function getChecked($id){
       global $wpdb;
       $sql ="SELECT CheckIn.ID FROM $this->tbl_check_in AS CheckIn 
              LEFT JOIN $this->tbl_event AS Event on CheckIn.event_id = Event.ID
              WHERE  CheckIn.guests_id = $id AND Event.status = 1";
