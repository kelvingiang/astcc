...
class Metabox_Special
{
    ...
    public function display($post)
    {
        ...
        //  $value                   =1;
        $getvalue            =  get_post_meta($post->ID, '_Metabox_special', true);
        $getorder            =  get_post_meta($post->ID, '_Metabox_order', true);
        $inputvalue            =  'on';
        echo '<p><label>' . __('次序') . '</label> '
            . "<input tpye='text' name='txt-order' id='txt-order' value='$getorder' />"
            . '</p>';
    }
    ...
    public  function save($post_id)
    {
        ...
        //     $active = 'off';
        // }
        // update_post_meta($post_id, '_Metabox_special', $active);
        if (empty($_POST['txt-order'])) {
            $orderValue = 0;
        } else {
            $orderValue = $_POST['txt-order'];
        }
        update_post_meta($post_id, '_Metabox_order', $orderValue);
    }
}
