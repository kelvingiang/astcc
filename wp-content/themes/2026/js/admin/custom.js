...
function myFunction($mess, $page, $action, $id) {
  if (confirm($mess)) {
    location.href =
...
// Ngày: 22/07/2026
// Chức năng: Tự động định dạng giờ (thêm dấu ":" sau 2 số đầu tiên) cho input class .type-time, đồng thời hỗ trợ phím xóa lùi thông minh (tự xóa ":" khi người dùng xóa số phút).
jQuery(".type-time").on("input", function (e) {
  var val = jQuery(this).val();
  var inputType = e.originalEvent ? e.originalEvent.inputType : "";
  // Kiểm tra xem người dùng có đang dùng phím xóa (Backspace/Delete) hay không
  var isDeleting =
    inputType === "deleteContentBackward" ||
    inputType === "deleteContentForward";

  if (isDeleting) {
    // Nếu người dùng xóa số phút làm chuỗi chỉ còn lại dạng "HH:", thì tự động xóa luôn dấu ":" thành "HH"
    if (val.length === 3 && val.slice(-1) === ":") {
      jQuery(this).val(val.slice(0, 2));
    }
  } else {
    // Nếu đang nhập, chỉ lấy các ký tự số
    var numbers = val.replace(/[^0-9]/g, "");
    if (numbers.length >= 2) {
      // Tự động chèn ":" sau 2 số đầu tiên
      jQuery(this).val(numbers.substring(0, 2) + ":" + numbers.substring(2, 4));
    } else {
