<?php
class Admin_Countries_Controler{
    public  function __construct() {
        add_action('init', array($this, 'countriesCusPost'));
        add_action('manage_edit-countries_columns', array($this, 'countriesManage_cols'));
        add_action('manage_countries_posts_custom_column', array($this, 'countriesRender_cols'));
    }
    
     public function countriesCusPost() {
        $labels = array(
            'name' => _x('各國通訊', 'suite'),
    ...
    public function countriesManage_cols($columns){
    ...
    public function countriesRender_cols($columns){
         global $post;
        switch ($columns){
            case 'hoitruong' :
