...
class Admin_About_Us_Controler {
...
// THAM SO THU 5 GOI DEN HAM HIEN THI GIAO DIEN TRONG MENU
        $icon = ICON_DIR . 'admin-icon.png';  // THAM SO THU 6 LA LINK DEN ICON DAI DIEN
...
// LOC DU LIEU KHI action = -1 CO NGHIA LA DANG LOI DU LIEU (CHO 2 TRUONG HOP search va filter)
        ...
        if(isPost()){
          require_once(MODEL_DIR.'about_us_model.php');
          $model = new Admin_About_Us_Model();
          $model->Save($_POST);
        }
        require_once ( VIEW_DIR . 'about_us_view.php');
    }
