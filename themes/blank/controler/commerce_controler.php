<?php
class Admin_Commerce_Controler{
    public  function __construct() {
        add_action('init', array($this, 'commerceCusPost'));
        add_action('manage_edit-commerce_columns', array($this, 'commerceManage_cols'));
        add_action('manage_commerce_posts_custom_column', array($this, 'commerceRender_cols'));
        
          add_filter('manage_edit-commerce_sortable_columns', array($this, 'custom_post_column_views_sortable'));
        add_filter('request', array($this, 'sort_views_column'));
    }

     public function commerceCusPost() {
        $labels = array(
            'name' => _x('各國商會', 'suite'),
    ...
    public function commerceManage_cols($columns){
    ...
    public function commerceRender_cols($columns){
        global $post;
        switch ($columns){
            case 'country':
   ...
   public function custom_post_column_views_sortable($newcolumn) {
