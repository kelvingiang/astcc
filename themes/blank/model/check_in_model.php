...
class Admin_Check_In_Model extends WP_List_Table {
    ...
    private function table_data() {
        ...
        if (getParams('s') != ' ') {
            ...
            $whereArr[] = "(m.full_name  LIKE '%$s%' or m.barcode LIKE '%$s%')";
        }
...
//        $linkDelete = wp_nonce_url($linkDelete, $action, $name);
    ...
    private function deleteImg($arrID) {
        ...
        if (!is_array($arrID)) {
            ...
            unlink(WB_DIR_IMAGES_GUESTS . $row['img']);
            unlink(WB_DIR_IMAGES_BARCODE . $row['barcode'] . '.png');
        } else {
    ...
    public function saveItem($arrData = array(), $option = array()) {
        ...
        if (!empty($_FILES['guests_img']['name'])) {
            ...
            $path = WP_CONTENT_DIR . DS . 'themes' . DS . 'blank' . DS . 'images' . DS . 'guests' . DS; // get function path upload img dc khai bao tai file hepler

            if (empty($this->_error) == true) {
                if (is_file(WB_DIR_IMAGES_GUESTS . $arrData['hidden_img'])) {
                    unlink(WB_DIR_IMAGES_GUESTS . $arrData['hidden_img']);
                }
                //=== upload hinh ==============================
