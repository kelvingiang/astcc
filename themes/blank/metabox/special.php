...
class Admin_Metabox_Special{
   ...
   //    echo '<p><i> active or inactive </i></p>';
       ...
       //  $value                   =1;
        ...
        $getorder            =  get_post_meta($post->ID, '_admin_metabox_order',true);
        $inputvalue            =  'on';
        $arr                       = array( 'id' => $inputID);
        $options                = array ('current_value' => $getvalue);        
        echo  '<p><label for ="'. $inputID. '">' .  __('現任')  .' : </label>'
       ...
       // 4 BON PHAN TREN DUNG DE BAO MAT KHI LUU METABOX TRONG WP 
        if($_POST['admin-metabox-special'] == 'on') { $active = 'on';} else {$active = 'off';}
        ...
        update_post_meta($post_id, '_admin_metabox_order', $orderValue);
